...
class PTX_Validate_FieldCompare extends Zend_Validate_Abstract {
    ...
    public function isValid($value) {
        if ($this->_operator && isset(self::$_tmp_values[$this->_key])) {
            ...
            throw new Zend_Validate_Exception('Field Compare Validator : operator does not specified(key=' .$this->_key . ')');
        } else if (!$this->_operator && !isset(self::$_tmp_operators[$this->_key])) {
            self::$_tmp_values[$this->_key] = $value;
        }
        ...
        if ($this->_key_value && $this->_operator) {
            ...
            throw new Zend_Validate_Exception('Field Compare Validator : operator not found(key=' . $this->_key .',op=' . $this->_operator . ')');
            return false;
        }
    ...
    protected function _error($messageKey = null, $value = null) {
        ...
        self::$_key_messages[$this->_key][$messageKey] = $this->_createMessage($messageKey, $value);
    }
