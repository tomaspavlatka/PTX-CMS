...
class Admin_MenuPlaceController extends Zend_Controller_Action {    
    ...
    public function init() {
        $this->view->h1 = $this->view->trans('Menu places');
    ...
    public function deleteAction() {
        ...
        // Message + redirect.
        if($rows > 0) {
            PTX_Message::setMessage4User('admin','done',$this->view->transParam('Menu place <b>~0~</b> has been deleted.',array('~0~'=>$placeData['name_'.$this->_locale]),false));
        } else {
            PTX_Message::setMessage4User('admin','warning',$this->view->transParam('Menu place <b>~0~</b> could not be deleted.',array('~0~'=>$placeData['name_'.$this->_locale]),false));
        }
    ...
    public function editAction() {
        ...
        // Form.
        $form = new Admin_Form_MenuPlace(array('id'=>$idPlace));
    ...
    public function indexAction() {
        $this->_redirect('/admin/menu-place/list');
    }
    ...
    public function listAction() {
        ...
        $select->columns(array('name_'.$this->_locale.' as name','status','id'));
        $select->where('status > -1')->order('name_'.$this->_locale.' asc');
    ...
    private function _addForm(Zend_Controller_Request_Http $request, Zend_Form $form) {
        if($request->isPost()) {
            if($form->isValid($request->getPost())) {
                ...
                $placesObj = new Admin_Model_MenuPlaces();
                $idPlace = $placesObj->save($formData);                
                
                PTX_Message::setMessage4User('admin','done',$this->view->transParam('Menu place <b>~0~</b> has been stored.',array('~0~'=>$formData['name'.$this->_locale]),false));
     ...
     * @param Zend_Controller_Request_Http $request
     * @param Zend_Form $form
     * @param Admin_Model_MenuPlace $placeObj
     * @return Zend_Form
     */
    private function _editForm(Zend_Controller_Request_Http $request, Zend_Form $form, Admin_Model_MenuPlace $placeObj) {
        if($request->isPost()) {
            if($form->isValid($request->getPost())) {
                $formData = $form->getValues();
                $placeObj->update($formData);
                
                PTX_Message::setMessage4User('admin','done',$this->view->transParam('Menu place <b>~0~</b> has been updated.',array('~0~'=>$formData['name'.$this->_locale]),false));
                $this->_redirect(PTX_Anchor::getAnchor('admin'));
            }
